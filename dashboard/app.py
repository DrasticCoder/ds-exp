...
st.caption(
    "Real-time dashboard for disease outbreak risk prediction, monitoring, "
    "and analysis."
)
...
with st.sidebar:
    ...
    if uploaded is not None:
        try:
            ...
            # Silently ignore errors - consider logging in production
            pass
    ...
    threshold = st.slider("Probability threshold for 'High Risk'", 0.0, 1.0, 0.5, 0.01)
    st.divider()
    st.write(
...
# Tabs
# ----------------------------
tab_pred, tab_explore, tab_analysis, tab_shap, tab_responsible, tab_fair, tab_drift = (
    st.tabs(
        [
            "🔮 Risk Prediction",
            "📊 Data Exploration",
    ...
    if inference_pipeline is None or EXPECTED_COLS is None:
        ...
        if df_in is not None:
            ...
            if gt_col:
                ...
                y_pred = (
                    (out["proba_high_risk"].fillna(0.0).values >= threshold).astype(int)
    ...
    if uploaded is None:
        ...
        try:
            ...
            if "Cases_Reported" in df.columns and "Deaths_Reported" in df.columns:
                ...
                df["case_fatality_rate"] = df["Deaths_Reported"] / df[
                    "Cases_Reported"
                ].replace(0, 1)
            ...
            if "Country" in df.columns and "Cases_Reported" in df.columns:
                ...
                country_cases = (
                    df.groupby("Country")["Cases_Reported"]
                    .sum()
...
with tab_analysis:
    ...
    if uploaded is None:
        ...
        try:
            ...
            if (
                "case_fatality_rate" not in df.columns
                and "Cases_Reported" in df.columns
                and "Deaths_Reported" in df.columns
            ):
                df["case_fatality_rate"] = df["Deaths_Reported"] / df[
                    "Cases_Reported"
                ].replace(0, 1)
            ...
            if (
                "cases_per_100k" not in df.columns
                and "Cases_Reported" in df.columns
                and "Population" in df.columns
            ):
                df["cases_per_100k"] = (
                    df["Cases_Reported"] / df["Population"]
                ) * 100000
                ...
                try:
                    ...
                    if hasattr(model, "feature_importances_"):
                        ...
                        numeric_features = [
                            c
                            for c in EXPECTED_COLS
                            if c not in ["Country", "Disease_Name"]
                        ]
                        ...
                        categories = {
                            ...
                            "Geographic": [
                                f for f in feature_names if f.startswith("Country_")
                            ],
                            "Disease_Type": [
                                f
                                for f in feature_names
                                if f.startswith("Disease_Name_")
                            ],
                        }

                        category_importance = {}
                        for cat, features in categories.items():
                            cat_imp = importance_df[
                        ...
                        st.dataframe(
                            importance_df[
                                ["Feature", "Importance_Pct", "Cumulative_Importance"]
                            ].head(25),
    ...
    if inference_pipeline is None or EXPECTED_COLS is None:
        ...
        if uploaded is not None:
            try:
                ...
                st.success(
                    f"Using uploaded data with {len(shap_data)} records for SHAP analysis"
                )
            except Exception as e:
        ...
        if shap_data is not None:
            try:
                ...
                numeric_features = [
                    c for c in EXPECTED_COLS if c not in ["Country", "Disease_Name"]
                ]
                categorical_features = ["Country", "Disease_Name"]
                ...
                st.write(
                    f"**Computing SHAP values for {sample_size} samples with {len(feature_names)} features**"
                )
                ...
                try:
                    ...
                    st.error(
                        "SHAP library not available. Please install it with: pip install "
                        "shap==0.44.1"
                    )
                ...
                if shap_available:
                    with st.spinner("Computing SHAP values... This may take a moment."):
                        if hasattr(model, "predict_proba"):
                            ...
                            explainer = shap.Explainer(
                                model.predict_proba, X_transformed
                            )
                            ...
                            if (
                                ...
                                and len(shap_values.values.shape) > 2
                            ):
                    ...
                    for i, (_, row) in enumerate(importance_df.head(5).iterrows()):
                        ...
                        st.write(
                            f"**{i+1}. {feature_name}** (Impact: {importance:.4f})"
                        )
            ...
            st.info(
                "Please upload a CSV file or ensure reference data is available to "
                "compute SHAP values."
            )
...
with tab_responsible:
    ...
    if uploaded is None and REF is not None:
        st.info(
            "Using reference dataset for SHAP analysis. Upload your own data for "
            "custom analysis."
        )
        base_data = REF.copy()
            ...
            if hasattr(model, "feature_importances_"):
                ...
                with col2:
                    ...
                    for i, row in importance_df.head(10).iterrows():
                        st.write(
                            f"{row.name + 1}. **{row['Feature']}**: {row['Importance_Pct']:.1f}%"
                        )
...
with tab_fair:
    ...
    if has_gt:
        ...
        st.write(
            "**High-risk rate gap vs overall** (positive = predicts 'High Risk' "
        ...
        st.info(
            ...
            "If you want accuracy/F1 per group, set a ground-truth column (e.g., "
            "`high_risk_outbreak`, `outbreak_risk`, `target`, or `label`)."
        )
...
with tab_drift:
    ...
    if REF is None:
        st.info(
            "Missing reference_sample.csv in artifacts. Upload a CSV to compare, or "
            "add reference file."
        )
        ...
        if cur is not None:
            ...
            if not num_cols:
                ...
                st.caption(
                    "• PSI ≥ 0.2: Significant drift, model retraining recommended (HIGH)"
                )
                st.caption(
                    "• KS p-value < 0.05: Statistically significant distribution difference"
                )
